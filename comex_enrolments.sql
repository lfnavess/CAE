...
SELECT
	...
	CASE
		...
		WHEN
			...
			OR "vt_enrolments"."endtime" < DATEADD(DAY, DATEDIFF(DAY, 0, GETDATE()), 0) AND "vt_courses"."active" = 0	THEN 4
		--																												THEN 5
		WHEN
			"vt_enrolments"."endtime" < DATEADD(DAY, DATEDIFF(DAY, 0, GETDATE()), 0)
			AND "enrolment_status"."lastaccess" IS NOT NULL																THEN 6
...
FROM(
		SELECT 
			MAX("vt_enrolments"."id") AS "enrolmentid",
			NULLIF(COUNT(*), 1) AS "enrolments"
		FROM
			"vt_enrolments"
	...
	LEFT JOIN(
		SELECT
			"enrolmentid",
			FLOOR(AVG("progress_measure") * 100) / 100                          AS "progress_measure",
			CAST(AVG(CASE WHEN "score_type" = 1 THEN "score" END) * 100 AS INT)	AS "pre_score",
			CAST(AVG(CASE WHEN "score_type" = 2 THEN "score" END) * 100 AS INT)	AS "post_score",
	...
	LEFT JOIN "vt_users"				ON "vt_users"."id" = "vt_enrolments"."userid"
	LEFT JOIN "vt_users" AS "requestby"	ON "requestby"."siteid" = 5 AND "requestby"."username" = SUBSTRING("vt_course_groups"."name", NULLIF(CHARINDEX('|',"vt_course_groups"."name"), 0) + 1, 20)
WHERE "vt_users"."siteid" = 5 AND "vt_enrolments"."creationtime" >= '20150101';